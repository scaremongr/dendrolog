...
static void checkWrapKnownDefect(LogListView& view)
{
    ...
    CHECK(offscreen > 0,
          QStringLiteral("ДЕФЕКТ, ПОХОЖЕ, ИСПРАВЛЕН: с переносом строк навигация больше не "
                         "уводит текущую строку за экран. Верните обычные "
                         "checkKeyboard/checkGeometry для этого view и уберите эту проверку."));
}

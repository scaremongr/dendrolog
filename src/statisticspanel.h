...
public:
    ...
    // Документ грузится (первичный разбор/индексация или дочитывание хвоста).
    // Пока true, автоматический сбор не запускается: срез по наполовину
    // загруженному файлу картины не даёт, через мгновение сменится, а на
    // большом документе стоит секунд процессорного времени. Переход в false
    // запускает ОДИН сбор — уже по всему файлу. Ручной ⟳ не подчиняется.
...
private:
    // Немедленный асинхронный пересбор. manual — по кнопке ⟳: считает даже
    // при незавершённой загрузке (явное действие пользователя сильнее
    // эвристики «ждём полный файл»).
    void startCollection(bool manual = false);
    ...
    // Данные менялись, пока шёл сбор. Такой пересбор ставится в очередь и
    // запускается ПОСЛЕ текущего: на большом документе сбор длится дольше
    // интервала дозаписи живого лога, и отмена «по каждому изменению»
    // означала бы, что панель не покажет результат никогда.
    bool      m_pendingRefresh = false;
    bool      m_loading = false;          // документ ещё грузится (см. setLoading)

...
public:
    ...
    //
    // Помимо этого явного режима работает «прилипание»: если вьюпорт УЖЕ стоит
    // на последней строке, дозагрузка в конец прокручивает его к новому концу
    // (см. isScrolledToBottom и handleRowsInserted). Разница в том, что явный
    // режим возвращает к хвосту, откуда бы ни смотрел пользователь, а
    // прилипание просто не даёт хвосту уехать, пока читают именно его.
    void setFollowTail(bool enabled);
    bool followTail() const { return m_followTail; }

    // Вьюпорт стоит у последней строки. Допуск — один шаг прокрутки (строка):
    // высоты строк уточняются лениво и двигают maximum между батчами, из-за
    // чего точное равенство value == maximum терялось бы на ровном месте.
    // Пока контент влезает целиком (maximum == 0), прокрутки не было — это НЕ
    // «пользователь у хвоста», иначе первый же батч начальной загрузки
